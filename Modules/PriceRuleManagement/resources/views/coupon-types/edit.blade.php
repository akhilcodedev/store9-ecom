...
@section('content')
    <div class="container">
        ...
        @if(isset($couponMode))
            <form id="edit_coupon_mode_form" method="POST" action="{{route('priceRule.cart.couponTypes.update', ['id' => $couponMode->id])}} ">
                @csrf
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            setTimeout(() => {
                                window.location.href = "{{ route('priceRule.cart.couponTypes.index') }}"; //Use named routes
                            }, 1500);
                        } else {
