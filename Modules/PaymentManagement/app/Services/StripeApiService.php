...
class StripeApiService
{
    ...
    public function __construct()
    {
        ...
        $this->client = null;
        if ($stripeObj) {
            $credentialValues = (isset($stripeObj->credentials) && $this->checkIsValidJSONString($stripeObj->credentials)) ? json_decode($stripeObj->credentials, true) : [];
            if (is_array($credentialValues) && (count($credentialValues) > 0)) {
                $this->publishableKey = $credentialValues['publishable_key'] ?? null;
                $this->secretKey = $credentialValues['secret_key'] ?? null;
                $this->webhookSecretKey = $credentialValues['webhook_secret_key'] ?? null;
            }
        }
